    ...
    try {
      ...
      if (options.trimVideoStart.enabled || options.trimVideoEnd.enabled) {
        ...
        const endTime = options.trimVideoEnd.enabled
          ...
          : duration;
        ffmpegCommand.push("-ss", startTime.toString(), "-to", endTime.toString());
      }
      ...
      if (options.changeHSLLightness.enabled) {
        ffmpegCommand.push(
          "-vf",
          `colorlevels=rimin=${options.changeHSLLightness.value / 100 + 1}:gimin=${
            options.changeHSLLightness.value / 100 + 1
          }:bimin=${options.changeHSLLightness.value / 100 + 1}`
        );
      }
